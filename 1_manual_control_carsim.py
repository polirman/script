...
class World(object):
    ...
    def restart(self):
        ...
        for actor in self.world.get_actors().filter(self._actor_filter):
            if actor.attributes.get('role_name') == self.actor_role_name:
                existing_vehicle = actor
                break

        if existing_vehicle:
            self.player = existing_vehicle
            print(f"Attached to existing vehicle with role name: {self.actor_role_name}")
        else:
            print(f"No vehicle with role name '{self.actor_role_name}' found. Spawning a new one.")
        ...
        if self.player is None:
            print("Failed to get a player vehicle. Exiting.")
            sys.exit(1)
    ...
    def destroy(self):
        ...
        if self.player is not None and self.player.is_alive:
            # We only destroy the player if we spawned it. A simple check is to see
            # if the script thinks it's the hero.
            # Note: This isn't foolproof if multiple scripts use 'hero' rolename.
            if self.player.attributes.get('role_name') == self.actor_role_name:
                self.player.destroy()
        self.player = None
